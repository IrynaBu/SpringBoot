$(function() {
  ...
  var springfox = {
    ...
    "securityConfig": function(onSuccess, onError) {
      $.ajax({
        url: this.getBaseUrl() + "/configuration/security",
        dataType: 'json',
        success: onSuccess,
        error: onError
      });
    ...
    "uiConfig": function(onSuccess, onError) {
      $.ajax({
        url: this.getBaseUrl() + "/configuration/ui",
        dataType: 'json',
        success: onSuccess,
        error: onError
      });
  ...
  function onSuccess(data) {
    window.swaggerUi = new SwaggerUi({
      ...
      validatorUrl: data.data.validatorUrl,
      supportedSubmitMethods: data.data.supportedSubmitMethods || ['get', 'post', 'put', 'delete', 'patch'],
      docExpansion: data.data.docExpansion || 'none',
      jsonEditor: JSON.parse(data.data.jsonEditor) || false,
      apisSorter: data.data.apisSorter || 'alpha',
      defaultModelRendering: data.data.defaultModelRendering || 'schema',
      showRequestHeaders: data.data.showRequestHeaders || true,
      timeout: data.data.requestTimeout,
